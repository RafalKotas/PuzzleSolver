...
import com.puzzlesolverappbackend.puzzleAppFileManager.nonogram.logic.helpers.common.NonogramFieldPlacingXHelper;
import com.puzzlesolverappbackend.puzzleAppFileManager.nonogram.logic.helpers.loggeneration.range.SequenceRangeCorrectionWhenPlacingXsLogHelper;
...
import static com.puzzlesolverappbackend.puzzleAppFileManager.nonogram.logic.NonogramLogicParams.SHOW_REPETITIONS;
...
public class ColumnXPlacementHelperImpl implements ColumnXPlacementHelper {
        ...
        for (int rowIdx = 0; rowIdx < logic.getNonogramRules().getHeight(); rowIdx++) {
            ...
            if (!isReachable) {
                ...
                if (isFieldEmpty(logic.getNonogramSolutionBoard(), fieldToExclude)) {
                    nonogramFieldPlacingXHelper.placeXAtGivenField(fieldToExclude, false);
    ...
    public void placeXsColumnAtTooShortEmptySequences(int columnIdx) {
        ...
        for (int rowIdx = 0; rowIdx < height - 1; rowIdx++) {
            if (!isFieldWithX(logic.getNonogramSolutionBoard(), new Field(rowIdx, columnIdx))) continue;
            ...
            if (!fittingSequences.isEmpty() && fittingSequences.equals(tooLongSequences)) {
                for (int rowIdxInEmptyRange = emptyRange.get(0); rowIdxInEmptyRange <= emptyRange.get(1); rowIdxInEmptyRange++) {
                    Field field = new Field(rowIdxInEmptyRange, columnIdx);
    ...
    public void placeXsColumnIfOWillMergeNearFieldsToTooLongColouredSequence(int columnIdx) {
        List<Integer> colouredFields = findColouredFieldsIndexesInColumn(logic.getNonogramSolutionBoard(), columnIdx);
