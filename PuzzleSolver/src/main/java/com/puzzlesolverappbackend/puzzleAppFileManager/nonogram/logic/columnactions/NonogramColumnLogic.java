...
import com.puzzlesolverappbackend.puzzleAppFileManager.nonogram.logic.helpers.solve.TooLongMergeFieldHelper;
...
public class NonogramColumnLogic extends NonogramLogicParams implements ColumnActions {
    ...
    public void correctColumnSequencesRangesWhenMatchingFieldsToSequences(int columnIdx) {
        ...
        List<List<Integer>> colouredRanges = collectColouredSequencesRanges(this.getNonogramSolutionBoard(), columnIdx, false);
        ...
        do {
            ...
            for (int colouredIdx = 0; colouredIdx < colouredRanges.size(); colouredIdx++) {
                ...
                for (int seqIdx = 0; seqIdx < columnSequencesRanges.size(); seqIdx++) {
                    ...
                    int sequenceLen = columnSequencesLengths.get(seqIdx);
            ...
            Map<Integer, List<Integer>> colouredRangeToSeqIdsBottom = new HashMap<>();
            for (int i = colouredRanges.size() - 1; i >= 0; i--) {
                ...
                colouredRangeToSeqIdsBottom.put(i, possibleSeqs);
            }

            int minAssignedSeqId = columnSequencesRanges.size();
            for (int i = colouredRanges.size() - 1; i >= 0; i--) {
                List<Integer> possibleSeqs = colouredRangeToSeqIdsBottom.get(i);
                if (possibleSeqs == null || possibleSeqs.isEmpty()) continue;
                ...
                colouredRangeToSeqIdsBottom.put(i, filtered);
            ...
            for (Map.Entry<Integer, List<Integer>> entry : colouredRangeToSeqIdsBottom.entrySet()) {
                List<Integer> possibleSeqs = entry.getValue();
                if (possibleSeqs == null || possibleSeqs.isEmpty()) continue;
    ...
    private void placeXsAroundLongestSequence(int columnIdx, List<Integer> xEdges, boolean onlyMatching) {
        ...
        for (int rowIdx : xEdges) {
            if (!isRowIndexValid(rowIdx)) continue;
    ...
    private void updateColumnSequenceAndExclude(int columnIdx,
        ...
        List<Integer> oldRange = this.getColumnsSequencesRanges().get(columnIdx).get(seqIdx);
        if (!newRange.equals(oldRange)) {
            ...
            if (triggeredByPlacingXs) {
                ...
                tmpLog = SequenceRangeCorrectionWhenPlacingXsLogHelper.generateLog(columnIdx, seqIdx, allRanges, newRange, columnState, lengths);
                addLog();
            }
        }
    ...
    private void checkDirectionAndPlaceXs(int columnIdx, boolean fromTop) {
        int start = fromTop ? 0 : this.getNonogramRules().getHeight() - 1;
        int end = fromTop ? this.getNonogramRules().getHeight() : 0;
    ...
    private List<Integer> getColouredFieldsRangeNearEmptySequenceFromTop(Field firstSequenceField) {
    ...
    private List<Integer> getColouredFieldsRangeNearEmptySequenceToTop(Field firstSequenceField) {
        List<Integer> colouredFieldsRange = new ArrayList<>();
        Field fieldToCheckO = new Field(firstSequenceField.getRowIdx(), firstSequenceField.getColumnIdx());
    ...
    private void evaluateAndMaybePlaceX(int columnIdx, List<Integer> emptyRange, List<Integer> colouredRange, boolean isFromTop) {
        ...
        for (int seqIdx = 0; seqIdx < columnSequencesLengths.size(); seqIdx++) {
            ...
            boolean firstCase = rangeLength(columnSequenceNearXRange) < rangeLength(emptyRange)
                    && rangeInsideAnotherRange(columnSequenceNearXRange, seqRange);
            boolean secondCase = rangeLength(columnSequenceNearXRange) >= rangeLength(emptyRange)
                    && totalLength <= sequenceLength && rangeInsideAnotherRange(columnSequenceNearXRange, seqRange);
    ...
    private boolean isRowRangeColoured(int columnIdx, List<Integer> rowRange) {
        ...
        return IntStream.rangeClosed(start, end)
                ...
                .allMatch(field -> isFieldColoured(nonogramSolutionBoard, field));
    }
