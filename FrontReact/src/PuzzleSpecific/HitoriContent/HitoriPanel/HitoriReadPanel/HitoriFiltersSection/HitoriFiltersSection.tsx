...
import { hitoriOptions, InitOptionsList, UpdateNumberValuedFilters, UpdateStringValuedFilters } from "../../../../../store/filters/hitori"
import { possibleHitoriOptions } from "../../../../../store/data/hitori"

interface OwnHitoriFiltersSectionProps {
...
const mapStateToProps = (state: AppState) => ({
    ...
    allSources: state.hitoriFiltersReducer.sources,
    allDifficulties: state.hitoriFiltersReducer.difficulties,
    allWidths: state.hitoriFiltersReducer.widths,
    allHeights: state.hitoriFiltersReducer.heights,
...
const mapDispatchToProps = (dispatch: Dispatch) => ({
    ...
    initOptions: (options: hitoriOptions) => 
...
type HitoriFiltersSectionPropsFromRedux = ConnectedProps<typeof connector>

type HitoriFiltersSectionProps = HitoriFiltersSectionPropsFromRedux & OwnHitoriFiltersSectionProps

const HitoriFiltersSection : React.FC<HitoriFiltersSectionProps> = ({hitorisList, showFilters, 
    initOptions, markFiltersCollected, updateStringFilters, updateNumberFilters,
    allSources, allDifficulties, allHeights, allWidths, selectedSources}) => {

    useEffect(() => {

        let initialOptions : hitoriOptions = {
            "source": [] as string[],
        ...
        for(let optionName of possibleHitoriOptions) {
    
            let optionNameCasted = optionName as keyof hitoriOptions
                ...
                <React.Fragment>
                    ...
                    }}>
                        {
                            Object.entries(checkBoxGroups).map(([label, value]) => {
                                return <CheckBoxGroup
                                    key={"hitori-filter-checkBoxGroup-" + label}
                                    label={label}
                    ...
                    }}>
                    {
                        Object.entries(sliders).map(([label, values]) => {
                            return (
                                <CustomMUISlider
                                    key={"hitori-filter-slider-" + label}
...
export default connect(mapStateToProps, mapDispatchToProps)(HitoriFiltersSection)